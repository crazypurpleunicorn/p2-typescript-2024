import { writeFile, writeFileSync } from "fs";
import { spacexLaunch, spacexLaunchType } from "./launches.js";

const head = (title: string, extrastring = "") => `
<head>
  <meta charset="UTF-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>${title}</title>
  <link rel="stylesheet" href=".${extrastring}/styles.css">
</head>`;

function createLaunchHtml(launch: spacexLaunch) {
  let headLaunch = head(launch.id, ".");
  let htmlLaunch = "";
  //add header
  htmlLaunch += `
  <header>
    <div class="svglogocontainer">
    <img src="data:image/png;base64," alt="Real Star link logo" class="svglogo">
    </div>
  </header>`;
  //add nav bar
  htmlLaunch += `
  <nav class="sticky">
        <div class="center maxwidth1050">
            <ul>
                <li>
                    <a href="#">Passed launches</a>
                </li>
                >
            </ul>
        </div>
    </nav>`;

    htmlLaunch += `
    <img src= ${launch.picture.large} class="specificimage"></img>
    <ul class="listacojonuda">
      <li> Status: ${launch.success}</li>
      <li>Name: ${launch.name}</li>
      <li>Date UTC: ${launch.dateUtc}</li>
      <li>Flight Number: ${launch.flightNumber}</li>
      <li>ID: ${launch.id}</li>
    </ul>`

  // for (let field in launch) {
  //   //get the value of the field ///Tres horas y no entiendo el error este de typescript de los huevos...en fin
  //   const value: spacexLaunchType = launch[field]
  //   // Create a list item for each field
  //   htmlLaunch += `
  //         <li>
  //           <span class="field">${field}</span>:
  //           <span class="value">${JSON.stringify(value)}</span>
  //         </li>
  //       `;
  

  return `
<html>
${headLaunch}
<body>
  ${htmlLaunch}
</body>
</html>`;
}

function createLaunchHtmlFile(launch: spacexLaunch) {
  let launchHtml = createLaunchHtml(launch);
  //no entiendo el writeFile aquí no me funciona
  writeFileSync(`./launchHtmls/${launch.id}.html`, launchHtml);
}
const renderLaunches = (launches: Array<spacexLaunch>) => {
  //Render main page

  let html = "";
  //add header
  html += `
  <header>
    <div class="svglogocontainer">
    <img src="data:image/png;base64," alt="Real Star link logo" class="svglogo">
  </header>`;
  //add nav bar
  html += `
  <nav class="sticky">
        <div class="center maxwidth1050">
            <ul>
                <li>
                    <a href="#">Passed launches</a>
                </li>
                >
            </ul>
        </div>
    </nav>`;
  for (const launch of launches) {
    if (launch.success) {
      //set success-true or success-false clase as a function of the launch's succes
      html += `
      <div class="launch">
      <img src="${launch.picture.small}" />
      <div class="data">
        <div class="name"><a href=${`./launchHtmls/${launch.id}.html`}>${
        launch.name
      }</a></div>
        <div class="name success-true">Mission status: <span>Success</span></div>
        <div class="email">Mission flightnumber: ${launch.flightNumber}</div>
        <div class="date">${launch.dateUtc}</div>
      </div>
    </div>`;
    } else {
      html += `<div class="launch">
      <img src="${launch.picture.small}" />
      <div class="data">
      <div class="name"><a href=${`./launchHtmls/${launch.id}.html`}>${
        launch.name
      }</a></div>
        <div class="name success-false">Mission status: <span>Failure</span></div>
        <div class="email">Mission flightnumber: ${launch.flightNumber}</div>
        <div class="date">${launch.dateUtc}</div>
      </div>
    </div>`;
    }
  }
  return html;
};

export const render = (launches: Array<spacexLaunch>) => {
  //Render and create a html for every launch
  launches.map((l) => createLaunchHtmlFile(l));

  return `
<html>
  ${head("Launches List")}
  <body>
    ${renderLaunches(launches)}
  </body>
</html>`;
};
